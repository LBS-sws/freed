...
<script>
    function downloadFileprom(mid, did, fid) {
        href = '<?php echo Yii::app()->createUrl('projectManage/filedownload');?>?mastId='+mid+'&docId='+did+'&fileId='+fid+'&doctype=PROM';
        window.open(href);
    }
    function downloadFileproinfo(mid, did, fid) {
        href = '<?php echo Yii::app()->createUrl('treatyInfo/filedownload');?>?mastId='+mid+'&docId='+did+'&fileId='+fid+'&doctype=PROINFO';
        window.open(href);
    }
</script>
<?php
	$doc = new DocMan($doctype,0,'ProjectManage');
