<?php
$this->pageTitle=Yii::app()->name . ' - AnalyzeProOne Form';
?>

<?php $form=$this->beginWidget('TbActiveForm', array(
'id'=>'AnalyzeProOne-form',
...
<section class="content">
	...
	<div class="btn-group" role="group">
		<?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
				'submit'=>Yii::app()->createUrl('analyzeProOne/index',array("index"=>$model->menu_id))));
		?>
	</div>
            <div class="btn-group pull-right" role="group">
                <?php echo TbHtml::button('<span class="fa fa-download"></span> '.Yii::t('dialog','Download'), array(
                    'submit'=>Yii::app()->createUrl('analyzeProOne/downExcel')));
                ?>
            </div>
	</div></div>
                    ...
                    <div class="col-lg-12" style="padding-top: 15px;">
                        <div class="row panel panel-default" style="border-color: #333">
                            ...
                            <div class="table-responsive">
                                <?php echo $model->analyzeProOneHtml();?>
                            </div>
                        </div>
                    </div>
                </div>
...
$js="
    $('.click-th').click(function(){
        ...
        if($(this).hasClass('active')){
            ...
            $('#analyzeProOne>thead>tr').eq(0).children().slice(startNum,endNum).each(function(){
                var width = $(this).data('width')+'px';
                $(this).width(width);
            });
            $('#analyzeProOne>thead>tr').eq(2).children().slice(startNum-contNum,endNum-contNum).each(function(){
                $(this).children('span').text($(this).data('text'));
            });
            $('#analyzeProOne>tbody>tr').each(function(){
...
";
$ajaxUrl = Yii::app()->createUrl('analyzeProOne/ajaxDetail');
$js.= "
