...
class Email {
    ...
    //添加收信人
    public function addEmailToProjectAndType($project_id,$emailType=array(1,2)){
        $emailType = implode(",",$emailType);
        $suffix = Yii::app()->params['envSuffix'];
        $rows = Yii::app()->db->createCommand()->select("username")
            ->from("freed$suffix.fed_project_email")
            ->where("project_id=:id and email_type in ({$emailType})",array(":id"=>$project_id))
            ->queryAll();
        if($rows){
            foreach ($rows as $row){
                $this->addEmailToLcu($row["username"]);
            }
        }
    }
    ...
    public function notEmailToLcu($lcu){
        $suffix = Yii::app()->params['envSuffix'];
        $row = Yii::app()->db->createCommand()->select("email, username")->from("security$suffix.sec_user")
            ->where("username=:username and email !='' and status='A'",array(":username"=>$lcu))
            ->queryRow();
        if($row){
            foreach ($this->to_addr as $key=>$email){
                if($email==$row["email"]){
                    unset($this->to_addr[$key]);
                }
            }
            foreach ($this->to_user as $key=>$username){
                if($username==$row["username"]){
                    unset($this->to_user[$key]);
                }
            }
        }
    }
    ...
    public function sent($uid="",$systemId="",$request_dt=""){
        ...
        if(!empty($this->to_user)){
            ...
            $message = $this->getBodyForHtml($this->message);
   ...
   protected function getBodyForHtml($html){
       $message="";
       if (strpos($html,'<body>')!==false){
           $list = explode("<body>",$html);
           $list = count($list)>=2?explode("</body>",$list[1]):$list;
           $message.=$list[0];
       }else{
           $message = $html;
       }
